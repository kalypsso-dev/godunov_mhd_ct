...
def do_plot(ini_filename, shock_tube_name, add_reference_data):
    ...
    ax1.semilogy(pos, rho, 'g-', label='rho')
    ax2.semilogy(pos, By, 'b-', label='By')
    ...
    if add_reference_data:
        ...
        ax1.semilogy(pos_ref, rho_ref, 'g--', label='rho ref')
        ax2.semilogy(pos_ref, By_ref, 'b--', label='By ref')
...
if __name__ == "__main__":
    ...
    shock_tube_name="Expansion problem II"
    parser = argparse.ArgumentParser(description='Display '+shock_tube_name+' plots.')
    parser.add_argument('--ini', type=str, default='test_mhd_expansion_II_2d.ini', help='ini parameter file')
